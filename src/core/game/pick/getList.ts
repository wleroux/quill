...
import {GAME_TIERS, getTier} from "@/model/game/GameTier";
...
export async function getList(message: APIMessage, minPlayers: number, maxPlayers: number): Promise<Result<GameList | undefined, string>> {
  ...
  const validGameTiers = GAME_TIERS.flatMap(tier => {
    ...
    return [tierPlayerCharacters.map(playerCharacters => playerCharacters[0])]
  });
