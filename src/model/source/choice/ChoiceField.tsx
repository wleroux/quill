...
export function ChoiceField({character, choice, decision, onChange}: {
  ...
  onChange: (fn: (value: Decision | undefined) => Decision | undefined) => void
}) {
  ...
  if (choice.type === "attribute" && (decision === undefined || decision?.type === "attribute")) {
    return <AttributeField character={character} choice={choice} decision={decision} onChange={(value) => onChange(_ => value)} />
  } else if (choice.type === "background-attribute" && (decision === undefined || decision?.type === "background-attribute")) {
    return <BackgroundAttributeField character={character} choice={choice} value={decision} onChange={(value) => onChange(_ => value)} />
  } else if (choice.type === "simple" && (decision === undefined || decision.type === "simple")) {
    return <SimpleField choice={choice} value={decision} onChange={(value) => onChange(_ => value)}/>
  } else if (choice.type === "expertise" && (decision === undefined || decision.type === "expertise")) {
    return <ExpertiseField value={character} choice={choice} decision={decision} onChange={(value) => onChange(_ => value)}/>
    ...
    return <MetamagicField value={character} choice={choice} decision={decision} onChange={(value) => onChange(_ => value)} />
  } else if (choice.type === "metamagic-replacement" && (decision === undefined || decision.type === "metamagic-replacement")) {
    return <MetamagicReplacementField value={character} choice={choice} decision={decision} onChange={fn => onChange(decision => {
      if (decision === undefined || decision.type === "metamagic-replacement") return fn(decision)
    })} />
